		...
		if(dump_flag && (i % 100 == 0)){
			if(myRank == 0){
				int k;
                    		...
                    		for(k = 1; k<p; k++){
                                	MPI_Recv(foreigners, number * (sizeof (struct particle)) / sizeof(double), MPI_DOUBLE, k, tag, MPI_COMM_WORLD, &status);
                                	printFile(foreigners, i,myRank,number);
                        	}
                	}else{
                        	MPI_Send(locals, number * (sizeof (struct particle)) / sizeof(double), MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
                	}
                }
	}
...
double randomValue(int type, int rank, int p){
	...
	switch(type){
		case POSITION:
			...
			break;
		case POSITION_X:
			value = randomRange((double)rank * (double)LIMIT / (double)p, (double)(rank + 1) * (double)LIMIT / (double)p);
			break;
